...
const Sidebar = ({ isOpen, onClose, weather, loading, position }) => {
    ...
    return (
        <div className={`absolute top-0 right-0 h-full w-full sm:w-[450px] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl z-[2000] transform transition-transform duration-500 cubic-bezier(0.4, 0, 0.2, 1) ${isOpen ? 'translate-x-0' : 'translate-x-full'} overflow-y-auto border-l border-white/20 dark:border-gray-700/30`}>
            ...
            <div className="sticky top-0 z-20 flex items-center justify-between p-5 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800/50">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="p-6 space-y-8">
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 text-white shadow-xl shadow-blue-500/20 p-6 group hover:shadow-2xl hover:shadow-blue-500/30 transition-all duration-300">
                            ...
                            <div className="relative z-10 flex justify-between items-center">
                                <div>
                                    <div className="flex items-start">
                                        ...
                                        <span className="text-3xl font-light mt-2 opacity-80">°</span>
                                    </div>
                            ...
                            <div className="relative z-10 mt-6 flex items-center gap-6 text-sm font-medium text-blue-50/80">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>L: {Math.round(weather.temp - 3)}°</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" />
                                    </svg>
                                    <span>H: {Math.round(weather.temp + 3)}°</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { label: 'Wind', value: `${weather.wind} km/h`, icon: '💨', color: 'text-cyan-500' },
                                { label: 'Humidity', value: `${weather.humidity}%`, icon: '💧', color: 'text-blue-500' },
                                { label: 'Pressure', value: `${weather.pressure} hPa`, icon: '📉', color: 'text-purple-500' },
                                { label: 'Clouds', value: `${weather.clouds}%`, icon: '☁️', color: 'text-gray-500' }
                            ].map((item, index) => (
                                <div key={index} className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-700/50 hover:bg-white dark:hover:bg-gray-800 transition-colors duration-200">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className="text-xl">{item.icon}</span>
                                        <span className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">{item.label}</span>
                                    </div>
                        ...
                        <div className="bg-white dark:bg-gray-800/30 rounded-3xl border border-gray-100 dark:border-gray-700/50 overflow-hidden">
                            ...
                            <div className="p-2">
                                <HourlyForecast coords={{ lat: position[0], lon: position[1] }} compact={true} />
                            </div>
                        </div>
