            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('wind') || 'Wind'}</span>
                    </div>
                    <div className="flex items-end justify-between">
                        <div>
                            <span className="text-3xl font-bold text-gray-900 dark:text-white">{(weather.wind.speed * 3.6).toFixed(1)}</span>
                        ...
                        <div className="relative w-12 h-12 border-2 border-gray-100 dark:border-gray-700 rounded-full flex items-center justify-center">
                            <div className="absolute top-0 text-[8px] text-gray-400">N</div>
                            <svg 
                ...
                <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('humidity') || 'Humidity'}</span>
                    </div>
                    <div className="flex flex-col gap-2">
                        <div>
                            <span className="text-3xl font-bold text-gray-900 dark:text-white">{weather.main.humidity}</span>
                        ...
                        <span className="text-xs text-gray-400 mt-1">Dew point: {(weather.main.temp - ((100 - weather.main.humidity)/5)).toFixed(1)}°</span>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex-1 bg-white dark:bg-gray-900 p-4 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-xs text-gray-500 uppercase">{t('visibility') || 'Visibility'}</p>
                                <p className="text-lg font-bold text-gray-900 dark:text-white">{(weather.visibility / 1000).toFixed(1)} km</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex-1 bg-white dark:bg-gray-900 p-4 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-xs text-gray-500 uppercase">{t('pressure') || 'Pressure'}</p>
                                <p className="text-lg font-bold text-gray-900 dark:text-white">{weather.main.pressure} hPa</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-3xl border border-orange-100 dark:border-gray-700 shadow-sm relative overflow-hidden">
                    <div className="flex justify-between items-center mb-2 relative z-10">
                        <span className="text-sm font-medium text-orange-800 dark:text-orange-300">{t('sunCycle') || 'Sun Cycle'}</span>
                        {/* Live Time Badge */}
                    ...
                    <div className="flex justify-center items-end h-24 relative z-10 mt-2">
                        <svg className="w-full h-full overflow-visible" viewBox="0 0 120 60">
                            {/* Arc Path */}
                            <path d="M 10 60 A 50 50 0 0 1 110 60" fill="none" stroke="currentColor" className="text-orange-200 dark:text-gray-600" strokeWidth="2" strokeDasharray="4 4" />
                            
                            {/* Sun Icon moving along path */}
                            <g transform={`translate(${sunX}, ${sunY})`}>
                                <circle r="6" className="text-orange-500" fill="currentColor" />
                                <circle r="10" className="text-orange-400 animate-pulse" fill="currentColor" fillOpacity="0.3" />
                            </g>
                        </svg>
                    ...
                    <div className="flex justify-between text-xs font-medium text-gray-500 dark:text-gray-400 mt-2 relative z-10">
                        <div className="flex flex-col items-start">
                            <span>Sunrise</span>
                            <span className="text-gray-900 dark:text-white">{formatTime(sunrise)}</span>
                        </div>
                        <div className="flex flex-col items-end">
                            <span>Sunset</span>
                            <span className="text-gray-900 dark:text-white">{formatTime(sunset)}</span>
                        </div>
                    </div>
                </div>
