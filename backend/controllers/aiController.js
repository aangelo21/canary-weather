...
export const chatWithAI = async (req, res) => {
    try {
        ...
        const weatherPromises = pois.map(async (poi) => {
            ...
            const weather = await fetchRealTimeWeather(poi.latitude, poi.longitude);
        ...
        const realTimeWeatherResults = (await Promise.all(weatherPromises)).filter(w => w !== null);
        
        // Format Alerts Context
        const alertContext = alerts.length > 0 ? alerts.map(a => 
            `- ⚠️ ${a.level || 'Alert'} (${a.phenomenon || 'General'}) in ${a.Location ? a.Location.name : 'Unknown'}.`
        ).join('\n') : "No active alerts.";

        // Format Weather Context
        const weatherContext = realTimeWeatherResults.map(w => 
