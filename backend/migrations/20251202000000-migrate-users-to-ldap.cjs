  ...
  async up(queryInterface, Sequelize) {
    ...
    const dropForeignKey = async (tableName, columnName) => {
      ...
      if (results && results.length > 0) {
        for (const row of results) {
          ...
          console.log(`Dropping constraint ${constraintName} on ${tableName}`);
          try {
            await queryInterface.removeConstraint(tableName, constraintName);
          } catch (e) {
            console.log(`Failed to remove ${constraintName}: ${e.message}`);
          }
        }
      } else {
        console.log(`No foreign key found for ${tableName}.${columnName}`);
    ...
    const changeCol = async (tableName) => {
      try {
        ...
        console.error(`Failed to change column on ${tableName}:`, err.message);
        throw err;
      }
    };
